    ...
    return new Promise((resolve, reject) => {
        ...
        db.serialize(() => {
            ...
            playerIds.forEach((playerId, index) => {
                stmt.run([
                    playerId, playerId, playerId, winIncrement, playerId, loseIncrement,
                    playerId, winIncrement, playerId, loseIncrement,
                    playerId, winIncrement, playerId, winIncrement, playerId, loseIncrement
                ], function(err) {
                    if (err) {
                        console.error(`❌ Error updating player ${playerId}:`, err);
                        hasError = true;
                    } else {
                        console.log(`✅ Updated stats for player ${playerId} (${index + 1}/${playerIds.length})`);
                    }
                            ...
                            if (hasError) {
                                ...
                                db.run('COMMIT', (commitErr) => {
                                    if (commitErr) {
                                        ...
                                        console.log(`✅ Successfully updated ${playerIds.length} players`);
                                        resolve();
                                    }
                                });
                            }
