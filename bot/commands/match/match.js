  ...
  bot.onText(/^\/match(?:\s+(.+))?$/, async (msg, match) => {
    ...
    const runScoreUpdate = async () => {
      ...
      try {
        ...
        // Generate AI summary for the match
        const aiSummary = await generateMatchSummary(updated);
    ...
    const runStatUpdate = async (action, playerNumStr, valueStr) => {
      ...
      if (isNaN(playerNum) || playerNum < 1) {
        sendMessage({
          msg,
          type: 'DEFAULT',
          message: MATCH.invalidPlayerNumber,
        });
        return;
      }
      ...
      if (!inMatch) {
        sendMessage({
          msg,
          type: 'DEFAULT',
          message: MATCH.playerNotInMatch.replace('{number}', playerNum),
        });
        return;
      }
      try {
    ...
    if (isSave) {
      ...
      try {
        ...
        const { homePlayers, awayPlayers, extraPlayers } =
          await buildPlayerEntries(teamA, teamB, team3C, allPlayers);
      ...
      try {
        ...
        // Generate AI summary if match has results
        let aiSummary = null;
        if (m.home_score != null && m.away_score != null) {
          aiSummary = await generateMatchSummary(m);
        }
